...
sudo dnf install kitty git vim htop fastfetch cronie network-manager-applet blueman unrar p7zip p7zip-plugins
```
...
sudo cp ~/ASM-D/.config/rofi/themes/* /usr/share/rofi/themes/
cp ~/ASM-D/.bashrc ~/
cp ~/ASM-D/.vimrc ~/
mkdir ~/Pictures && cp -r ~/ASM-D/Wallpapers ~/Pictures/
sudo rm -rf ~/ASM-D
